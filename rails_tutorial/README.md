...
初めに、ユーザ情報を保存するためのデータ構造を作る  
モデル：データモデルとして扱うデフォルトのデータ構造

- DBを用いて、データを永続化する
- Active Recordを用いて、DBとやり取りする
  - Railsのライブラリ
...
- `generate model`コマンドで、モデルを作成する
- コントローラ名は複数形、モデルは名には単数系を用いる
- マイグレーションにより、DBの構造をインクリメンタルに変更する手段が提供される
...
- `change`メソッドの集まり
  ...
  - `create_table`
    - Railsのメソッド
    - ブロック変数`t`は、tableの頭文字
  - マジックカラム
    - `created_at`
    - `updated_at`

`db:migrate`コマンドにより、マイグレーションの適用を行う
...
- `User.first`
- `User.all`
  - 返り値のクラスは、`ActiveRecort::Relation`

#### 6.1.5 ユーザオブジェクトを更新する

- 属性を個別に代入する
  - 最後に`save`を実行する
  - `reload`を実行すると、DBの情報を再読み込みし、変更が取り消される
- `update_attributes`を使う
  - `user.update_attributes(name: "uptest", email: "date@test.org")`
  - 属性のハッシュを受け取る
  - 更新と保存を同時に行う
  - 検証に1つでも失敗すると、呼び出しは失敗
  - `update_attribute`
...
DBに保存される前に、nameとemailフィールドが存在することを保証する
...
- `:unique`
  - `uniqueness`と`case_sensitive`
- `@user.dup`：同じ属性を持つユーザを複製

Active Recordは、DBレベルでは一意性を保証していない

- 2度押し
- DBのemailカラムにインデックスを追加、一意であるように
  - Full-table Scanの解決にもなる

DBのアダプタが、常に大文字と小文字を区別するインデックスを使っているとは限らない

- DB保存前に、全ての文字列を小文字にする
- call backを使う

### 6.3 セキュアなパスワードを追加する

パスワード（とその確認）をハッシュ化してDBに保存する

- ハッシュ化
  - ハッシュ関数を用いて、不可逆なデータとする
  - ハッシュ化されたパスワードはユーザ認証で用いる
- ユーザ認証
  - パスワードの送信
  - ハッシュ化
  - DB内のハッシュ化された値との比較
...
- `has_secure_password`
  - セキュアなパスワードの実装を行う
    - ハッシュ化したパスワードをDB内の`password_digest`という属性に保存できるように
    - `password`, `password_confirmation`が使えるように
      - 存在性と一意性に関するバリデーションも追加
    - `authenticate`メソッドが使えるように
  - `password_digest`という属性がモデルに含まれていることが必要

#### 6.3.2 ユーザがセキュアなパスワードを持っている
...
- DBにユーザを一人追加する
- `has_secure_password`の効果についても確認する
  - `authenticate`メソッド
    - 以下を比較する
      - 引数に渡された文字列をハッシュ化した値
      - DB内にある`password_digest`カラムの値
    - 第8章では、このメソッドを使って、ログインする

### 6.4 最後に

## 第7章　ユーザ登録

- HTMLフォームを使ってWebアプリに登録情報を送信
- ユーザを新規作成してDBに保存
- ユーザを表示するためのプロフィールを表示できるように

### 7.1 ユーザを表示する

ユーザの名前とプロフィール写真を表示するためのページを作成する

#### 7.1.1 デバッグとRails環境

- Sassのミックスイン機能(`box_sizing`)を使用している

#### 7.1.2 Usersリソース

Railsコンソールで、ユーザレコードが登録されている状態を作っておく

- RESTアーキテクチャの習慣に従う
  - データの作成、表示、更新、削除をリソース（resource）として扱う
  - `POST`, `GET`, `PATCH`, `DELETE`を各アクションに割り当てる 

`resources :users`により、RESTfulなUsersリソースで必要となるすべてのアクションが利用できるようになる 

#### 7.1.3 debuggerメソッド

`debugger`を書く、`Ctrl + d`で抜ける
